...
class SignatureWidget extends Widget
{
    ...
    function __construct($d_or_annot = null)
    {
        ...
        switch (func_num_args()) {
            case 0:
                $this->_cPtr = new_SignatureWidget();
                break;
            default:
                $this->_cPtr = new_SignatureWidget($d_or_annot);
        }
    }
    ...
    static function Create($doc, $pos, $field_name_or_field = null)
    {
        switch (func_num_args()) {
            case 2:
                $r = SignatureWidget_Create($doc, $pos);
                break;
            default:
                $r = SignatureWidget_Create($doc, $pos, $field_name_or_field);
        }
        if (is_resource($r)) {
            $c = substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
            if (class_exists($c)) return new $c($r);
