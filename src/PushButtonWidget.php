...
class PushButtonWidget extends Widget
{
    ...
    function __construct($d_or_annot = null)
    {
        ...
        switch (func_num_args()) {
            case 0:
                $this->_cPtr = new_PushButtonWidget();
                break;
            default:
                $this->_cPtr = new_PushButtonWidget($d_or_annot);
        }
    }
    ...
    static function Create($doc, $pos, $field_name_or_field = null)
    {
        switch (func_num_args()) {
            case 2:
                $r = PushButtonWidget_Create($doc, $pos);
                break;
            default:
                $r = PushButtonWidget_Create($doc, $pos, $field_name_or_field);
        }
        if (is_resource($r)) {
